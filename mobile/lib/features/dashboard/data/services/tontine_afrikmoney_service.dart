...
class TontineAfrikmoneyDepositIntent {
  final String id;
  final double amount;
  final String status;
  ...
  const TontineAfrikmoneyDepositIntent({
    required this.id,
  ...
  factory TontineAfrikmoneyDepositIntent.fromMap(Map<dynamic, dynamic> map) {
    return TontineAfrikmoneyDepositIntent(
      id: '${map['id'] ?? ''}',
...
class TontineAfrikmoneyService {
  ...
  Future<TontineAfrikmoneyDepositIntent> createDeposit(double amount) async {
    ...
    if (!intent.hasPaymentUrl) {
      throw const ApiException(
        'Le lien Afrikmoney est indisponible pour ce paiement.',
        null,
        ApiErrorType.server,
      );
    }
  ...
  Future<TontineAfrikmoneyDepositIntent> fetchDepositIntent(
    String intentId,
  ) async {
    final payload = await _apiClient.get(
      '/tontine/afrikmoney/deposits/$intentId',
    );
    return TontineAfrikmoneyDepositIntent.fromMap(_asMap(payload));
  }
